...
- **Keep somebody out of one group type.** Exclude a participant from the room
  draw and leave them in every other group type. The allocation honours it, the
  board shows who is excluded and puts them back, and every count on the screen
  agrees. Excluding somebody out of a locked group offers to unlock the place it
  empties.
- **A backup now holds the whole event:** exclusions, the settings and rules an
  organiser chose, check-in columns and their ticks, notes, history, and the
  original dates.
- **Move a whole workspace to your own server with one command,** with a trial run
  that writes nothing, and one damaged event no longer stopping the rest.
- **Exclusions in both people exports** — a column in the spreadsheet from the
  People page, and in a person's own data export.
- **A roster PDF page for everybody who is not in a group,** split between those
  you excluded and those the allocation could not place.

### Fixed

- **A damaged backup file costs you the damaged line, not the whole restore,** and
  the restore says what it had to skip, shorten or fall back on.
- **Counts that add up.** Somebody who cancelled or was removed no longer counts as
  excluded, and no figure can go below zero.
- **A user can be deleted whatever they have done.** Their drafts go with them;
  what they published stays, marked as written by a removed user.
- **The registration form says what is wrong in your own language,** beside the box
  that is wrong, instead of a line of program code or the browser's own English
  message.
- **Dates and times in the format you chose,** everywhere, on a 24-hour clock and
  in the event's own time zone.
- **Screens that had stopped fitting:** the people list, the pop-out panel, narrow
  windows, and scrollbars you can see and grab again.
- **Notes say who wrote them,** and the history no longer shows moves that never
  happened.
- Wording corrections throughout, in all six languages.

### Changed

- **Detailed logging is off by default.** Query logging wrote participant names and
  addresses into the server log during ordinary use. Set `LOG_LEVEL=DEBUG` when you
  need it back.
- **Team members and their roles are never taken from a backup.** Invite the team
  again on the receiving server.
- **The hidden per-person limit on group codes is retired.** No screen could set it
  and nothing enforced it.

### For self-hosters

- **Two database changes apply themselves** on the first start of the new image.
  Nothing to run by hand; take your usual backup first, as with any upgrade.
- **Older backup files still restore,** and a file made by this release can still
  be read by an older Moimio.
