...
<!-- TODO: screenshot at docs/assets/allocation-board.png — the AllocationBoard for one category mid-allocation -->
...
Five passes, in order. Each pass tries to honour soft constraints; both passes always respect hard constraints (gender restriction, capacity).
...
**Pass 5 — Classify unplaced.** Anyone the engine couldn't place is tagged with a reason: cluster too big to split, gender unknown for restricted units, no capacity, etc. These show up in the **Needs allocation** banner above the unit grid.
...
| **Group remaining participants by gender** | When placing uncoded participants in Pass 4b, prefer same-gender groupings. Helps homogenise units when no other constraint applies. |
...
At small event sizes (under 50 participants), a single large group_code cluster can produce visible imbalance — by design. The engine prioritises cluster cohesion over room balance. If you want even distribution, use a **mark with "Spread evenly"** behaviour instead of a group_code. Group codes mean *"definitely keep together"*; spread-evenly marks mean *"distribute across units"*.
